...
void UFlareCompanyAI::UpdateTrading()
{
	...
	IdleCargoCapacity = 0;
	TArray<UFlareSimulatedSpacecraft*> IdleCargos = AITradeHelper::FindIdleCargos(Company);
...
*/
		...
		SectorDeal BestDeal = AITradeHelper::FindBestDealForShip(Ship, WorldResourceVariation);

		if (BestDeal.Resource)
		{
			AITradeHelper::ApplyDeal(Ship, BestDeal, &WorldResourceVariation);
		}
		else
		{
#ifdef DEBUG_AI_TRADING
